...
## Resolving shared dependencies 解决共享依赖项
...
### Unshared libraries (the npm approach) 未共享库（npm 方法）
...
1. `collection` defines some `Dictionary` class.
   `collection` 定义了一些 `Dictionary` 类。
2. `widgets` gets an instance of it from its copy of `collection` (`2.3.5`). It then passes it up to `my_app`.
   `widgets` 从其 `collection` 副本（ `2.3.5` ）获取它的实例。然后，它将其传递给 `my_app` 。
3. `my_app` sends the dictionary over to `templates`.
   `my_app` 将词典发送给 `templates` 。
4. That in turn sends it down to *its* version of `collection` (`2.3.7`).
   然后，它将其发送到其 `collection` 版本（ `2.3.7` ）。
5. The method that takes it has a `Dictionary` type annotation for that object.
   接收它的方法为该对象具有 `Dictionary` 类型注释。
...
### Version lock (the dead end approach) 版本锁定（死胡同方法）
...
### Version constraints (the Dart approach) 版本约束（Dart 方法）
...
## Constraint solving for exported dependencies 解决导出的依赖项的约束
...
### You can have disjoint constraints 您可能具有不连贯的约束
...
### You can have ranges that don’t contain a released version 您可能具有不包含已发布版本的范围
...
### You can have an unstable graph 您可能有一个不稳定的图
