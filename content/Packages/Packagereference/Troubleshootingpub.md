...
## 发布软件包时出现“403”错误 Getting a “403” error when publishing a package 
...
## 发布软件包时出现“UnauthorizedAccess”错误 Getting an “UnauthorizedAccess” error when publishing a package 
...
## Pub 构建因 HttpException 错误而失败 Pub build fails with HttpException error  
...
## 在公司防火墙后 pub get 失败 Pub get fails from behind a corporate firewall 
...
## 登录后无法访问 Localhost - Localhost unreachable after sign-in 
...
2. 在终端窗口中，运行 `dart pub publish` 。

3. In the browser window that comes up, sign in.

4. 在弹出的浏览器窗口中，登录。
   The browser is redirected to a *new localhost URL* (`http://localhost:<port>?code=...`) but complains that the URL isn’t reachable.

5. 浏览器会重定向到新的 localhost URL（ `http://localhost:<port>?code=...` ），但会抱怨无法访问该 URL。

6. Copy the *new localhost URL* from the browser.

7. 从浏览器复制新的 localhost URL。

8. In another terminal window in the same container or on the same host as the one where `dart pub publish` was called, use the `curl` command to complete sign-in using the *new localhost URL*:

9. 在同一容器中的另一个终端窗口中，或在调用 `dart pub publish` 的同一主机上，使用 `curl` 命令使用新的 localhost URL 完成登录：

   ```
   $ curl 'http://localhost:<port>?code=...'
   ```

## 尝试查找软件包时出现套接字错误 Getting a socket error trying to find a package 
