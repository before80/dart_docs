...
## Sound and unsound null safety 健全和不健全的空安全
...
4. Add a [language version comment](https://dart.dev/guides/language/evolution#per-library-language-version-selection) to the top of any Dart files that you don’t want to consider during your current migration:
   在您当前迁移期间不想考虑的任何 Dart 文件顶部添加语言版本注释：
...
5. Migrate the code of each Dart file, using the analyzer to identify static errors.
   使用分析器识别静态错误，迁移每个 Dart 文件的代码。
   Eliminate static errors by adding `?`, `!`, `required`, and `late`, as needed.
   根据需要添加 `?` 、 `!` 、 `required` 和 `late` ，消除静态错误。

## Testing or running mixed-version programs 测试或运行混合版本程序
