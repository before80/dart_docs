...
## Implementing a simple worker isolate 实现简单的辅助进程隔离
...
### Running an existing method in a new isolate 在新隔离区中运行现有方法
...
1. Call `run()` to spawn a new isolate (a [background worker](https://dart.dev/language/concurrency#background-workers)), directly in the [main isolate](https://dart.dev/language/concurrency#isolates) while `main()` waits for the result:
   调用 `run()` 以生成新的隔离区（后台工作进程），直接在主隔离区中，同时 `main()` 等待结果：
...
1. Pass the worker isolate the function you want it to execute as its first argument. In this example, it’s the existing function `_readAndParseJson()`:
   将工作隔离区传递给您希望其作为其第一个参数执行的函数。在此示例中，它是现有函数 `_readAndParseJson()` ：
...
1. `Isolate.run()` takes the result `_readAndParseJson()` returns and sends the value back to the main isolate, shutting down the worker isolate.
   `Isolate.run()` 获取结果 `_readAndParseJson()` 返回并向主隔离区发送值，从而关闭工作隔离区。
2. The worker isolate *transfers* the memory holding the result to the main isolate. It *does not copy* the data. The worker isolate performs a verification pass to ensure the objects are allowed to be transferred.
   工作隔离区将保存结果的内存传输到主隔离区。它不会复制数据。工作隔离区执行验证过程以确保允许传输对象。
...
## Sending multiple messages between isolates with ports 使用端口在隔离之间发送多条消息
...
### `ReceivePort` and `SendPort` `ReceivePort` 和 `SendPort`
...
#### Step 1: Define the worker class 步骤 1：定义工作者类
...
#### Step 2: Spawn a worker isolate 步骤 2：生成工作者隔离区
...
#### Step 3: Execute code on the worker isolate 步骤 3：在工作者隔离区执行代码
...
#### Step 4: Handle messages on the main isolate 步骤 4：在主 isolate 上处理消息
...
#### Step 5: Add a completer to ensure your isolate is set-up 步骤 5：添加一个完成器以确保您的隔离区已设置
...
#### Step 2: Create a `RawReceivePort` in the `Worker.spawn` method 步骤 2：在 `Worker.spawn` 方法中创建一个 `RawReceivePort`
...
#### Step 3: Spawn a worker isolate with `Isolate.spawn` 步骤 3：使用 `Isolate.spawn` 生成一个工作者隔离
...
#### Step 4: Complete the isolate setup process 步骤 4：完成隔离设置过程
...
#### Step 5: Handle multiple messages at the same time 步骤 5：同时处理多条消息
