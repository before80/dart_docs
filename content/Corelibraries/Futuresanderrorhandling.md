...
## The Future API and callbacks Future API 和回调
...
## Examples of using then() with catchError() 结合 catchError() 使用 then() 的示例
...
### catchError() as a comprehensive error handler catchError() 作为综合错误处理程序
...
### Error handling within then() then() 中的错误处理
...
### Errors in the middle of a long chain 长链中间的错误
...
### Handling specific errors 处理特定错误
...
## Async try-catch-finally using whenComplete() 使用 whenComplete() 进行异步 try-catch-finally
...
### Completing the Future returned by whenComplete() 完成由 whenComplete() 返回的 Future
...
### Errors originating within whenComplete() 源自 whenComplete() 的错误
...
## Potential problem: failing to register error handlers early 潜在问题：未能尽早注册错误处理程序
...
## Potential problem: accidentally mixing synchronous and asynchronous errors 潜在问题：意外混合同步和异步错误
...
### Solution: Using Future.sync() to wrap your code 解决方案：使用 Future.sync() 包装您的代码
