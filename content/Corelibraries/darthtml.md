...
您可以通过创建新元素并将其附加到 DOM 来添加到现有 HTML 页面。以下是如何创建一个段落 (
...
#### Adding, replacing, and removing nodes 添加、替换和删除节点
...
#### Manipulating CSS styles 操作 CSS 样式
...
使用 `*element*.on*Event*.listen(*function*)` 添加事件处理程序，其中 `*Event*` 是事件名称， `*function*` 是事件处理程序。
...
### Using HTTP resources with HttpRequest 使用 HttpRequest 与 HTTP 资源进行交互
...
### Sending and receiving real-time data with WebSockets 使用 WebSocket 发送和接收实时数据
...
#### Handling WebSocket events 处理 WebSocket 事件
